...
model <- neuralnet(cfpb_score ~ ., data = train_data, err.fct="sse")
plot(model)
...
model2 <- neuralnet(cfpb_score ~ ., data = train_data, hidden = 5)
...
concrete_norm$household_size <- normalize(as.numeric(concrete_norm$household_size))
