...
The cfpb scores from the Federal Consumer Protection Bureau and the Federal Reserve System appear to follow a normal distribution with similar sample statistics mean median and standard deviation. 
...
The survey results were sampled using a consistent weight for cfpb score. The resulting dataset includes 4000 surverys from each survey data set. 
...
The survey data is comprehensive with 217 variables in the CFPB Survey and 372 variables in the Federal Reserve survey. To reduce the solution space the survey data was pre-filtered to only include variables that i thought would be likely to contribute to the effectiveness of the model. A subset of the variable exploration is included in this document additional visualization and analysis can be found in the accompanied DATA607_Plot document and the DATA607_STEP document. Based on the analysis and some initial regression models using backwards and forwards step elimination of variables. 
...
- age_7cat (ppagecat) - Age 7 Categories
...
From the initial exploration of the dependent variable, Financial Well-Being Scale (cfpb_score) follows the normal distribution with a cluster of results on the high and low end of scale. The median and the mean are fairly close so the distribution appears to be symmetric. The structure of the distribution suggest that there might be an opportunity to explore the top range of the distribution and the lower end of the distribution separately. This seems consistent with how the poor and the wealthy experience and navigate the economy.
...
- Low correlation across numeric values - There are limited numeric values in the survey data. The numeric data that exists is not heavily correlated. This is somewhat surprising given that at least 4 of the numeric values are defined to reflect financial knowledge or financial well being. The only two variable that display a correlation above 0.5 are the cfpb score for scaled financial wellness (FWBscore) and the Lusardi and Mitchell financial knowledge Summative scale scale scorescore (LMscore).

- Correlation varies by education and income level - Even the overall correlation between numeric variables is low there is a difference in correlation across education levels and income groups. There is an avenue of futher analysis if we segmented the population using those variables. 
...
The top categories for house values, morgage and income start at \$150,000, \$200,000 and \$150,000. The lowest categorise for house value, motgage and income include \$150,000, \$50,000 and \$20,000. The lowest categories and highest categories for these variables lickley exibit different behavior and economic engagemet. 
...
The initial review of the comparative visualizations for race and cfpb score seem to indicate that race may not be a primary driver of financial well being. There is some correlation between race and wellness score but the streangth of that relationship is not direclty evident in the following graphs.  
...
The data set from the Federal Reserve includes several questions that produce variable with overlapping measures. This includes the variables that meeasure education level, employment, house hold composition and ecomomic cercumstances. As part of the modeling selection process we will look to rationalize the list of variables so that we do not icnlude several variables that measure the same underlying characteristic. 

The Federal Reserve survey data includes some sharp contrasts in cfpb score based on the survey respondents perceptions and their economic circumstances. These survey responses had higher contributiosn to the regression model. 

Similar to the survey data from the CFPB a portion of the respondands declined to answere specific questions. We will filter out incomplete cases as required by the model being used. 


**Observations**

- Few numeric variables with limited correlations. - similar to the CFPB survey data the Federal Reserve survey data reflects low correlation between the numeric variables in the data set. It should also be noted that the FED data set include very few unique numeric variable. The overwhelming proportion of survey questions are designed around answer that can be placed in categories. The numeric numbers that exist primarily focus on house hold composition. 
...
The Economic circumstance variables (econ_fin_ok, econ_pay_exp400, econ_skip_med) are heavily represented in the federal reserve model. This is not surprising given the nature of the questions that define the cfpb score. It does suggest that given the same demographic and economic characteristics survey respondents may report differing financial wellness results. 

The box plots and the histogram of cfpb scores categorized by economic circumstance variables highlight the impact of these variables on the cfpb score of the survey respondents. It would indicate the survey respondents are aware of their current financial wellness circumstance. The alignment of the decision to forgo medical care based on on cfpb score highlights some real world impacts financial wellness. 

It should be noted that ven though the box plots for cfpb score categorized by survey respondents perception of their economic circumstances there are several outlive records. As you can see from the grouped histogram there is distinct shift in the distribution upwards for survey respondents who feel they are on firmer footing financial. There are also outlier data that cluser near the top end and bottom end of the histogram.
...
This analysis will focus on a linear regression model as the foundation of the conclusions. However one of the goals for this project was to explore different data science models. To those ends we will touch on a random forest model and neural network model that will utilize the same data set to generate a predictive model. 
...
The analysis started with a review of the survey code book to identify candidate variables for the analysis. Relieying on the survey findings, and industry knowledge I conducted an initial selection of the variables. This was just used as a starting point, additional analysis could be conducted to refine the intial lists of variables that are included but the focus of this assignment was on exploring data manipulation, modeling. 
...
Model stats
- r-squared for the model is 0.403
...
The regression results indicate some predictive 
...
In reviewing the analysis what jumps out is the impact of early decisions on the overall outcome. The technical execution of the analysis is important but early decison on data to include or exclude in the analysis. This analysis utilized several models with each model generating slightly different results. 

I had not expected to resolve any long standing debates in the financial industry but it has been surprising how many different avenues for analysis or questons arrose from this discussion. 
